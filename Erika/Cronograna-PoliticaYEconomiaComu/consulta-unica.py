#!/usr/bin/env python3
"""RAG mínimo sobre el cronograma de la materia Politíca y Economía de la Comunicación.

Qué hace RAG acá: en vez de mandarle al modelo todo el cronograma, buscamos
primero las pocas entradas parecidas a la pregunta y le mandamos SOLO esas. El
modelo no "sabe" el cronograma: lo lee en el momento, en el prompt.
...
Uso:
    python3 sasha/calendario-rag/consulta-unica.py "¿Cuándo es el 1er parcial?"
"""
...
SISTEMA = (
    "Sos un asistente sobre el cronograma de la materia Politíca y Economía de la Comunicación "
    "de la UNSAM. Respondé SOLO con lo que digan los fragmentos del cronograma "
    "que te paso; no inventes ni supongas nada que no esté ahí. Si los "
    "fragmentos no contienen la respuesta, contestá exactamente: "
    "«No lo sé, eso no figura en el cronograma». Si la contienen, escribí una "
...
def partir_en_entradas(texto):
    """[1]  Un fragmento por entrada del cronograma, con la clase adelante.

    Una entrada por fragmento (en vez de ventanas de N palabras) hace que la
    búsqueda se note eligiendo entre varias opciones. La clase va adelante
    para que el fragmento se entienda solo: "Clase 1 — 03/08
    Modalidad: Virtual...".
    """
    fragmentos = []
    mes = None
    for linea in texto.splitlines():
        ...
        if linea in ENCABEZADOS:
            ...
            fragmentos.append(f"{clase} — {linea}")
        # Sin clase todavía, la línea es del encabezado del archivo: la salteamos.
    return fragmentos
